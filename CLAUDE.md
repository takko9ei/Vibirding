...
完整架构、目录结构、数据结构、接口契约见 @docs/architecture.md
—— 那是唯一事实来源。
...
- IMPORTANT: 一次只实现一个切片(架构文档第10节 S1–S7)，
  不要一次写多个切片。
...
- 代码里用清晰的英文注释；关键逻辑写完后，用中文逐段解释它的原理。
- 用中文跟我交流。
- 倾向最小实现：跑通当前切片即可，不要提前加架构文档"可选进阶"
  里的东西(MCP / 多 agent / 本地模型 / 上下文压缩)。
