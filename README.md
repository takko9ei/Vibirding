...
当前“记一笔”页面已经可以选择多张 JPEG、输入整篇笔记、查看并编辑拆分草稿、调整照片归属，
再用唯一的“确认写入”按钮保存。解析失败会保留输入供重试；写入结果会分别列出成功项和失败
原因。`/observations` 管理页目前仍是 3.9 的静态骨架，将在 3.11 接通。

前端自动验收可在 `frontend/` 中运行：
...
- **2.6 已完成并提交**：增加原始需求承诺的 6 条 v2 固定离线 eval。
- **3.10 已实现、待 review**：输入流程已接通上传、解析预览、草稿编辑、照片归属和确认写入。
