...
【当前能力说明】本期暂无图片输入，bird_id（图片鉴种）尚未接入，但 range_check 已可用：
- 第2、3条里的“图片鉴定”暂不可用；
- 第4条（无种名也无图片）：应调用 range_check(place, date) 取当地当季物种清单，在清单内挑选与外形描述匹配的种；range_check 不可用时（如未知地点、网络失败、空清单）再退回纯鸟类学知识推断。
因此现在实际会走到第1条（用户指定）或第4条（描述推断 + range_check 核验）。
