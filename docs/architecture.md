...
├── Vibirding/                    # 主包
│   ├── __init__.py
│   ├── config.py                # 路径、模型名、从环境变量读 API key
│   ├── schemas.py               # ★所有数据结构(pydantic)，最先锁
│   ├── llm/
│   │   ├── client.py            # provider 中立的模型客户端
...
> 这些是整个系统的"血型"。先定死它们，多 agent、记忆、eval 才能干净地插进来。用 pydantic。

**ToolCall** — 模型发出的一个工具请求

```
id: str            # 配对用，对应 tool_result
name: str          # 工具名
input: dict        # 模型填的参数
```
...
**ModelResponse** — `llm/client` 归一化后的模型响应（屏蔽不同 provider 差异）
...
| `llm/client.py` + `llm/mock.py` | 模型适配 + 离线 mock                                            | `src/anthropic-adapter.ts` + `src/mock-model.ts` |
...
- 实现：`AnthropicClient`（真 API）、`MockClient`（脚本化，按预设依次返回 ModelResponse）。
- `MockClient` 是关键：让你**不花一分钱、不连真模型**就能把整个循环测通。
...
input_schema: dict        # JSON Schema，告诉模型怎么填参数
...
| 工具         | risk      | 作用                         | 备注                                                                                               |
| ------------ | --------- | ---------------------------- | -------------------------------------------------------------------------------------------------- |
| `bird_id`    | read      | 照片 URL → 候选鸟种 + 置信度 | **可替换适配器**；动工前先确认所选服务有没有公开 API，没有就退回 Merlin/eBird 或托管模型，上层不变 |
| `read_log`   | read      | 查历史观测（按地点/种/日期） | 用于"我去年在这儿见过啥""这季节这里有没有这种"                                                     |
...
cli 读入笔记 → messages=[{user: 笔记}]
  → run_agent_turn:
      回合1: llm.complete → stop_reason=tool_use, 请求 bird_id
             registry.execute(bird_id) [read,自动allow] → 候选鸟种
             追加 tool_call + tool_result;trace×2
      回合2: llm.complete → 可能再请求 read_log 核验季节 → 同上
...
**`run_evals.py`** —— 对每条任务跑 agent（用 `MockClient` 或低温真模型），比对：结构化字段是否匹配、是否调了该调的工具、是否乱调写入。输出**通过率 + 逐条 pass/fail**。
...
| 切片 | 内容                                                                         | 验收                                       |
| ---- | ---------------------------------------------------------------------------- | ------------------------------------------ |
| S1   | `schemas` + `MockClient` + `loop` + `registry` + 一个假 `read_log` + `trace` | 离线、零成本，循环能跑通，trace 打印出每步 |
| S2   | `AnthropicClient` 接真模型，能产出结构化 `Observation`                       | 真模型跑通一条笔记                         |
| S3   | `bird_id` 真适配器（先验证 API）                                             | 带照片能拿到候选种                         |
| S4   | `memory/log` 的 append/query + `append_log` 写入 + 权限闸                    | 能写日志、写前要确认、能查回来             |
| S5   | `budget` 步数上限 + 工具报错容错                                             | 死循环/报错不会失控                        |
| S6   | `evals`：10–15 条用例 + 通过率                                               | 一条命令出通过率                           |
| S7   | `cli` 打磨 + `README` + `DECISIONS.md`                                       | 别人能 clone 跑起来                        |

> S1 用 MockClient 把循环逻辑和真模型解耦，是整条路最省钱、最好 debug 的起点。**先把脑子（循环）调通，再接嘴（真模型）和手（真 API）。**

---

## 11. 可选进阶（面试谈资，做完 v1 再说，别提前背上）

- **核验子 agent（多 agent）**：一个专职 agent 拿 `bird_id` 结果 + `read_log` 的地点/季节信息，二次判断"这个种在这个时间地点合不合理"，给 `flags`。这是把单 agent 升级成多 agent 协同最自然的一步。
- **季节/分布知识工具**：接一个鸟种分布数据源，让核验有据可依。
- **本地模型**：给 `LLMClient` 加一个走 OpenAI 兼容端点的实现，接 llama.cpp/vllm，`--local` 开关。**别一开始碰这个**。
- **大工具结果移出 prompt** / **SQLite 替代 JSONL**：数据量大了再说。

---

## 12. 给 vibe coding 的三条铁律（细节下一轮展开）

1. 每个切片开工前，把本文档 + 该切片要碰的契约发给 AI，**不要让它自由发挥结构**。
2. 一次只让它实现一个模块，你读懂 diff 再进下一个；每个能跑的切片 `git commit` 一次。
3. 边做边往 `DECISIONS.md` 记取舍（为什么 JSONL 不用数据库？为什么写入过闸？）——这是你"思考"的显性化。
