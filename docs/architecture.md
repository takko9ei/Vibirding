...
**2.1–2.5 和 3.1–3.8 已完成、验证并提交，Web 视觉与响应式方向也已确认。3.9 React
基础工程已经实现并通过验证，当前等待 review。** 本切片只建立 Vite + React + TypeScript、
两个固定路由、共享视觉 token、类型化 API client 和本地开发代理；页面目前只有可辨认的静态
工作区结构，review 前不得开始其他切片。review 并提交 3.9 后，先补齐原始需求已列出但旧实施表
遗漏的 2.6 v2 eval，再开始 3.10 输入流程。
...
- `npm run typecheck` 与 `npm run build` 必须通过；保留 v1 离线 eval 13/13。3.9 review 前不得
...
- 新增独立的 v2 离线 eval 数据集与 runner，覆盖多物种拆分、共享上下文、同种多图、按
  `species_id` 图文匹配、未匹配照片自动成草稿，以及 ambiguous/unmapped 等可见失败。
...
当前执行顺序固定为：**review/提交 3.9 → 2.6 → 3.10 → 3.11 → 3.12 → 4**。
...
| 2.6 v2 固定 eval | **待做；下一开发切片** | 按第 9 节增加 v2 数据集与离线 runner，不改生产能力。 | v2 用例全通过；2.1–2.5 回归通过；v1 仍 13/13；停下 review。 |
...
| 3.9 React 基础工程 | **已实现；当前 review** | Vite、React、TypeScript、双路由、设计 token、API client 与代理。 | typecheck/lint/build、双路由与代理通过；review 后单独提交。 |
| 3.10 React 输入流程 | **待做；后续第 2** | 接入上传、parse、草稿编辑、照片归属、确认写入和部分成功展示。 | 纯文本/纯照片/混合、loading、错误重试和防重复提交通过；停下 review。 |
| 3.11 React 管理流程 | **待做；后续第 3** | 接入筛选、列表、详情、物种联想、编辑和二次确认删除。 | 空态/失败/编辑/删除与媒体展示可用；停下 review。 |
| 3.12 前端总体验收 | **待做；后续第 4** | 两页真实联调以及视觉、键盘、触控和响应式验收。 | 核心流程 E2E；1024/736/360px 无页面横向溢出；停下 review。 |
| 4 v2 收尾发布 | **待做；后续第 5** | 更新文档、最终全量回归并打 `v2.0` tag；不擅自增加部署范围。 | 文档/代码/测试一致，用户确认后创建 tag。 |
