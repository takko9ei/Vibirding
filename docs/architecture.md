...
| `range_check` | read | place + date → 该地当季合理出现的**物种清单**（数据源 eBird） | 季节/分布核验的**正主**；模型从清单里挑与外形描述匹配的种。与 `read_log` 区别：range_check 是**权威**物候/分布数据，read_log 只是**个人历史/弱先验** |
...
> `range_check` 待开工再解决（仅登记）：① 地名→坐标：先用"常去观测点预存坐标表"；② eBird 名单可能过长：按近期 `back=N` 天或按目标科收窄；③ 中日文鸟名：用 eBird taxonomy 的 `locale` 参数。
