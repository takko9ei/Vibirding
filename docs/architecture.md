...
**2.1–2.5 和 3.1–3.9 已完成、验证并提交，Web 视觉与响应式方向也已确认。此前实施表遗漏的
2.6 v2 固定 eval 已经补齐并通过验证，当前等待 review。** 本切片只增加固定离线数据集、runner
和评分，不修改生产 API、数据库 schema 或 React 页面；review 前不得开始 3.10 输入流程。
...
当前执行顺序固定为：**review/提交 2.6 → 3.10 → 3.11 → 3.12 → 4**。
...
| 2.6 v2 固定 eval | **已实现；当前 review** | 按第 9 节增加 6 条 v2 数据与离线 runner，不改生产能力。 | v2 6/6；2.1–2.5 为 156/156；v1 13/13；review 后单独提交。 |
...
| 3.10 React 输入流程 | **待做；下一开发切片** | 接入上传、parse、草稿编辑、照片归属、确认写入和部分成功展示。 | 纯文本/纯照片/混合、loading、错误重试和防重复提交通过；停下 review。 |
