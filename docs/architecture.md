...
**2.1 文本拆分和 2.2 照片预处理已完成并提交。2.3 物种名录与 dry-run 匹配已实现，
正在等待 review/commit。** 本切片创建 `species` 表、给 `observations` 增加可空
`species_id`，导入并解析 eBird 物种名录，只输出图文配对方案。本轮不得继续引入媒体/
批量 observation 写入、未匹配照片自动建记录、FastAPI 或 React；review 前不开始 2.4。
...
│   ├── tools/                            # 继承并后续扩展批量能力
│   ├── services/                         # parse / 第 2.3 步 matching + taxonomy；后续 media
